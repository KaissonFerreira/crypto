...
# and `pip install crypto_01` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "crypto_01", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n crypto_01 -f environment.docker.yml
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "crypto_01", "YOUR_CONSOLE_SCRIPT"]
